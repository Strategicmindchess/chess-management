...
import { Trophy, Calendar, Flame, Star, BookOpen, Link2, MessageSquareQuote, Loader2 } from 'lucide-react';
import { useStudentLeaderboard } from '@/hooks/use-student-leaderboard';

export function StudentLeaderboardClient() {
  const { data, isLoading, error } = useStudentLeaderboard();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        {hasLinkedAccounts && (
          <RefreshStatusBadge
            lastRefreshedAt={refreshStatus.lastRefreshedAt ? new Date(refreshStatus.lastRefreshedAt) : null}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/80 mt-0.5">
                <strong>{puzzleSolverAward.student.user.name}</strong>
                {' '}with {puzzleSolverAward.totalPuzzlesSolved} puzzles solved!
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {monthlyData.calculatedAt && (
                  <span className="text-[10px] text-slate-400">
                    Calculated: {new Date(monthlyData.calculatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Kolkata' })}
                  </span>
                )}
              </div>
              <LeaderboardTable
