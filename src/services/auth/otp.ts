...
export async function issueOtp(userId: string, purpose: OtpPurpose): Promise<string> {
  try {
    ...
    console.error("OTP Issue DB Error:", error);
    throw new Error("Could not issue OTP due to database error");
  }
}
...
export async function verifyOtp(
  ...
  try {
    ...
    console.error("OTP Verify DB Error:", error);
    return 'not_found';
  }
}
...
export async function canResendOtp(userId: string, purpose: OtpPurpose): Promise<boolean> {
  try {
    ...
    console.error("OTP Resend Check DB Error:", error);
    return false;
  }
}
